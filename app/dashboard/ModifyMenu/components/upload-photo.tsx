...
function useImage() {
    const inputRefs = useRef<{ [areaId: number]: { [itemId: number]: React.RefObject<HTMLInputElement> } }>({});
    const [images, setImages] = useState<{ [areaId: number]: { [itemId: number]: { name: string, url: string } } }>({});

    const handleUpload = (areaId: number, itemId: number, e: ChangeEvent<HTMLInputElement>) => {
    ...
    const getImage = (areaId: number, itemId: number) => {
        return images[areaId] && images[areaId][itemId] ? images[areaId][itemId] : null;
    };
